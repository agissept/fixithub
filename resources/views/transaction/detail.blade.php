<x-app-layout>
    ...
    @if($transaction->status === \App\Http\Enum\TransactionStatus::WAITING_CONFIRMATION->name)
        <div class="bg-white rounded-[11px] p-5 mt-5">
            ...
            <form method="POST" class="flex justify-end mt-4" action="{{route('transaction.update.progress', [$transaction->id])}}">
                @csrf
                <x-danger-button name='transaction_status' value="{{ \App\Http\Enum\TransactionStatus::REJECTED->name }}"
                                 class="mr-2">
                    Tolak
                </x-danger-button>
                <x-primary-button name='transaction_status' value="{{ \App\Http\Enum\TransactionStatus::CONFIRMED->name }}">Terima
                </x-primary-button>
            </form>
        </div>
    ...
    @else
        <form class="bg-white rounded-[11px] p-5 mt-5 block" method="POST"
              action="{{route('transaction.update.progress', [$transaction->id])}}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mt-4">
                ...
                <select id="transaction-status" name="transaction_status"
                    ...
                    <option value="{{ \App\Http\Enum\TransactionStatus::WAITING_FOR_SERVICE->name }}">Dalam antrian servie
                    </option>
